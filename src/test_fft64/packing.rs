...
use backend::{Encoding, FFT64, Module, ScratchOwned, Stats, VecZnxOps};
use sampling::source::Source;
use std::time::Instant;

use crate::{packing::StreamPacker, reverse_bits_msb};
...
fn packing() {
    ...
    let mut scratch: ScratchOwned = ScratchOwned::new(
        GLWECiphertext::encrypt_sk_scratch_space(&module, ct_size)
            ...
            | StreamPacker::add_scratch_space(&module, ct_size, auto_key_size, rank),
    );
    ...
    gal_els.iter().for_each(|gal_el| {
        let mut key: AutomorphismKey<Vec<u8>, FFT64> =
            AutomorphismKey::alloc(&module, basek, k_auto_key, rows, rank);
