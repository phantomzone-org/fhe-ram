// use poulpy_core::{AutomorphismKey, GGSWCiphertext, GLWECiphertext, GLWESecret, TensorKey};
...
use poulpy_core::{
    layouts::{
        prepared::{GGLWEAutomorphismKeyPrepared, GGSWCiphertextPrepared, GGLWETensorKeyPrepared, GLWESecretPrepared, PrepareAlloc}, GGLWEAutomorphismKey, GGLWETensorKey, GGSWCiphertext, GLWECiphertext, GLWESecret
    },
};
...
impl<D: Data + AsMut<[u8]> + AsRef<[u8]>> Coordinate<D> {
    ...
    pub(crate) fn encrypt_sk<DataSk: Data + AsRef<[u8]>>(
        ...
        let sk_glwe_prepared: GLWESecretPrepared<Vec<u8>, FFT64Avx> = sk.prepare_alloc(&module, scratch1);
        ...
        izip!(self.value.iter_mut(), self.base1d.0.iter()).for_each(|(coordinate, base)| {
            ...
            coordinate.encrypt_sk(module, &scalar, &sk_glwe_prepared, source_xa, source_xe, scratch1);
    ...
    /// * `scratch`: scratch space provider.
    pub(crate) fn invert<DataOther: Data + AsRef<[u8]>, DataAK: Data + AsRef<[u8]>, DataTK: Data + AsRef<[u8]>>(
        &mut self,
        module: &Module<FFT64Avx>,
        ...
        let auto_key_prepared: GGLWEAutomorphismKeyPrepared<Vec<u8>, FFT64Avx> = auto_key.prepare_alloc(&module, scratch1);
        let tensor_key_prepared : GGLWETensorKeyPrepared<Vec<u8>, FFT64Avx>  = tensor_key.prepare_alloc(&module, scratch1);

        izip!(self.value.iter_mut(), other.value.iter()).for_each(|(value, other)| {
            value.automorphism(module, other, &auto_key_prepared, &tensor_key_prepared, scratch);
        });
...
impl<D: Data+AsRef<[u8]>> Coordinate<D> {
    ...
    pub(crate) fn product<DataRes: Data + AsMut<[u8]> + AsRef<[u8]>, DataA: Data + AsRef<[u8]>>(
        ...
        self.value.iter().enumerate().for_each(|(i, coordinate)| {
            let (_, scratch1) = scratch.take_scalar_znx(module.n(), 1);
            let coordinate_prepared: GGSWCiphertextPrepared<Vec<u8>, FFT64Avx> = coordinate.prepare_alloc(&module, scratch1);
...
impl Base1D {
    ...
    pub(crate) fn recomp(&self, decomp: &Vec<u8>) -> u32 {
        ...
        self.0.iter().enumerate().for_each(|(i, base)| {
            value |= ((decomp[i] as u32) << sum_bases) as u32;
            sum_bases += base;
        });
        value
    }
...
mod tests {
    ...
    fn base1d_decomp_recomp_roundtrip() {
        ...
        for value in test_values {
            ...
            assert_eq!(decomp.len(), 3, "Decomposition should produce 3 elements for value {}", value);
            for &elem in &decomp {
                assert!(elem < 16, "Decomposed element {} should be < 16 for value {}", elem, value);
            }
        }
    }
    ...
    fn base2d_decomp_recomp_roundtrip() {
        ...
        for value in test_values {
            ...
            // Also verify decomposition properties
            assert_eq!(decomp.len(), 4, "Decomposition should produce 4 elements for value {}", value);
            for &elem in &decomp {
                assert!(elem < 16, "Decomposed element {} should be < 16 for value {}", elem, value);
            }
        }
    ...
    fn get_base_2d_functionality() {
        ...
        assert_eq!(value2, recomp, "Roundtrip failed for get_base_2d with value {}", value2);
        
        // Verify decomposition properties
        assert!(decomp.len() >= 1, "Decomposition should produce at least 1 element");
        for &elem in &decomp {
    ...
    fn base1d_edge_cases() {
        ...
        assert_eq!(empty_base.recomp(&vec![] as &Vec<u8>), 0);
    ...
    fn base1d_comprehensive_roundtrip() {
        ...
        let test_values = vec![
            0, 1, 2, 3, 4, 5, 10, 15, 16, 17, 31, 32, 63, 64, 127, 128, 255, 256, 511, 512, 1023, 1024, 2047, 2048, 4095
        ];
    ...
    fn base2d_comprehensive_roundtrip() {
        ...
        let test_values = vec![
            0, 1, 15, 16, 31, 32, 63, 64, 127, 128, 255, 256, 511, 512, 1023, 1024, 2047, 2048, 4095, 4096, 8191, 8192, 16383, 16384, 32767, 32768, 65535
        ];
    ...
    fn base1d_different_sizes() {
        ...
        for base in bases {
            ...
            for value in test_values {
                let decomp = base.decomp(value);
                let recomp = base.recomp(&decomp);
                assert_eq!(value, recomp, "Roundtrip failed for base {:?} with value {}", base.0, value);
            }
        }
    }
    ...
    fn base2d_edge_cases() {
        ...
        assert_eq!(empty_base2d.recomp(&vec![] as &Vec<u8>), 0);
