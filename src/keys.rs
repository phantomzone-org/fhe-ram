...
use poulpy_core::{
    layouts::{
        GGLWEAutomorphismKey, GGLWETensorKey, GLWECiphertext, GLWESecret
    },
};
...
pub fn gen_keys(params: &Parameters) -> (GLWESecret<Vec<u8>>, EvaluationKeys) {
    ...
    let mut source_1: Source = Source::new([1u8; 32]); // TODO: Create from random seed
    let mut source_2: Source = Source::new([1u8; 32]); // TODO: Create from random seed
...
mod tests {
    ...
    use poulpy_hal::{
        api::{ScratchOwnedBorrow, VecZnxCopy, VecZnxAutomorphismInplace},
    ...
    fn gen_keys_builds_complete_auto_keys_set() {
        ...
        let mut ct_in: GLWECiphertext<Vec<u8>> = GLWECiphertext::alloc(module.n(), basek, k_ct, rank);
        let mut pt: GLWEPlaintext<Vec<u8>> = GLWEPlaintext::alloc(module.n(), basek, k_pt);

        let pt_data: Vec<i64> = (0..module.n()).map(|i| (i % k_pt as usize) as i64).collect();
        pt.data.encode_vec_i64(basek, 0, k_pt, &pt_data, u8::BITS as usize);

        let mut scratch: ScratchOwned<FFT64Avx> = ScratchOwned::alloc(GGLWEAutomorphismKey::encrypt_sk_scratch_space(module, basek, k_ct, rank));
        let sk_prepared: GLWESecretPrepared<Vec<u8>, FFT64Avx> = sk.prepare_alloc(module, scratch.borrow());
        let mut source_xa: Source = Source::new([3u8; 32]);
        let mut source_xe: Source = Source::new([4u8; 32]);
        ct_in.encrypt_sk(module, &pt, &sk_prepared, &mut source_xa, &mut source_xe, scratch.borrow());
        ...
        for &gal_el in &gal_els {
            ...
            let auto_key_prepared: GGLWEAutomorphismKeyPrepared<Vec<u8>, FFT64Avx> = auto_key.prepare_alloc(module, scratch.borrow());

            let mut ct_out: GLWECiphertext<Vec<u8>> = GLWECiphertext::alloc(module.n(), basek, k_ct, rank);
            ...
            let mut pt_expected: GLWEPlaintext<Vec<u8>> = GLWEPlaintext::alloc(module.n(), basek, k_pt);
            ...
            assert_eq!(pt_expected.data, pt_out.data, "Automorphism failed for Galois element {}", gal_el);
            ...
            if gal_el != 1 {
                assert_ne!(ct_out, ct_in, "Ciphertext unchanged for non-identity Galois element {}", gal_el);
            } else {
                // For identity element, ciphertext should remain the same
                assert_eq!(ct_out, ct_in, "Ciphertext changed for identity Galois element");
            }
        }
    }
}
